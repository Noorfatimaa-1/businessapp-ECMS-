...
//loading data
bool loadproducts(string pdtnames[],float pdtprices[],int pdtquantity[],int &number_of_pdts,int &no_of_orders,string order_names[],float order_prices[],int order_quantities[],int &totalprice)
{
    ifstream fin;
    fin.open("pdts.txt");
    if(!fin)
...
//saving data
bool savepdts(string pdtnames[],float pdtprices[],int pdtquantities[],int &number_of_pdts,int &no_of_orders,string order_names[],float order_prices[],int order_quantities[],int &totalprice)
{
...
void deleteproducts(string pdtnames[],int pdtquantity[],float pdtprices[],int &number_of_pdts)                           //function for deleting products by the seller
{
    ...
    cout<<"Enter the password of the system then the deletion we will process your request."<<endl;
    cin>>delete_pwd;
    if(delete_pwd=="2abc")
    {
...
void updatingproducts(string pdtnames[],int pdtquantity[],float pdtprices[],int &number_of_pdts)                //function for updating products
{
    ...
    if(productnum>=1&&productnum<=number_of_pdts)
    {
        ...
        if(update==1||update==2||update==3)
        {
            if(update==1)
            {
                cout<<"CURRENT name OF PRODUCT IS : "<< pdtnames[productnum-1];
                cout<<endl;
                cout<<"Enter new name : ";
                cin.ignore();
            ...
            {
                cout<<"CURRENT quantity OF PRODUCT IS : "<< pdtquantity[productnum-1];
                cout<<endl;
            ...
            {
                cout<<"CURRENT price OF PRODUCT IS : "<< pdtprices[productnum-1];
                cout<<endl;
...
void listingout(string pdtnames[],int pdtquantity[],float pdtprices[],int &number_of_pdts)                    //function for listing the products out
{
    ...
    else
    {
      for(int i=0;i<60;i++)
        ...
        cout<<endl;
        cout<<endl;
        cout<<setw(15)<<left<<"Seriel no."<<setw(18)<<left<<"Names"<<setw(15)<<left<<"Price"  <<setw(15)<<right<<"Quantity: "<<endl;
        for(int i=0;i<65;i++)
                {
                    cout<<"-";               
                }
        ...
        for(int i=0;i<number_of_pdts;i++)
            {
                cout<<setw(15)<<left<<(i+1)<<setw(18)<<left<<pdtnames[i]<<setw(8)<<left<<pdtprices[i]<<"PKR"<<setw(15)<<right<<pdtquantity[i]<<endl;
                for(int i=0;i<65;i++)
                {
                cout<<"-";               
                }
...
void vieworders(string order_names[],float order_prices[],int order_quantities[],int &no_of_orders)                     //seller can view the orders
{
    ...
    for(int i=0;i<35;i++)
        ...
        for(int i=0;i<no_of_orders;i++)
        {
            cout<<setw(15)<<left<< i+1 <<setw(18)<<left<<order_names[i]<<setw(8)<<left<<order_prices[i]<<"PKR"<<setw(16)<<right<<order_quantities[i];
            cout<<endl;
            for(int i=0;i<65;i++)
            {
                cout<<"-";               
...
void showingsales(string order_names[],float order_prices[],int order_quantities[],int &no_of_orders ,int totalprice)
{
    if(no_of_orders>0)
    {
        ...
        cout<<endl;
        cout<<endl;
        cout<<setw(15)<<left<<"Sr. no."<<setw(18)<<left<<"Names"<<setw(16)<<left<<"Prices"<<setw(14)<<right<<"Quantity";
        cout<<endl;
        for(int i=0;i<65;i++)
                {
                    cout<<"-";               
        ...
        for(int i=0;i<no_of_orders;i++)
        {
            cout<<setw(15)<<left<< i+1 <<setw(18)<<left<<order_names[i]<<setw(8)<<left<<order_prices[i]<<" PKR"<<setw(15)<<right<<order_quantities[i];
            cout<<endl;
            for(int i=0;i<65;i++)
            {
                cout<<"-";               
...
void cart(string order_names[],float order_prices[],int order_quantities[],int &no_of_orders)                 //function to print cart on console
{
    ...
    {
    cout<<setw(15)<<left<<"Sr. no."<<setw(18)<<left<<"Names"<<setw(17)<<left<<"Prices"<<setw(13)<<right<<"Quantity";
    cout<<endl;
    for(int i=0;i<65;i++)
            {
                cout<<"-";               
    ...
    for(int i=0;i<no_of_orders;i++)
    {
        cout<<setw(15)<<left<< i+1 <<setw(18)<<left<<order_names[i]<<setw(7)<<left<<order_prices[i]<<"PKR"<<setw(17)<<right<<order_quantities[i];
         cout<<endl;
         for(int i=0;i<65;i++)
            {
                cout<<"-";               
...
void checkout(string order_names[],float order_prices[],int order_quantities[],int &no_of_orders,int totalprice)       //checkout function start
{
    if(no_of_orders>0)
    {
        ...
        cout<<endl;
        cout<<setw(15)<<left<<"Sr. no."<<setw(18)<<left<<"Names"<<setw(16)<<left<<"Prices"<<setw(12)<<right<<"Quantity";
        cout<<endl;
        for(int i=0;i<60;i++)
            {
                cout<<"-";               
        ...
        for(int i=0;i<no_of_orders;i++)
        {
            ...
            cout<<setw(15)<<left<< i+1 <<setw(18)<<left<<order_names[i]<<setw(7)<<left<<order_prices[i]<<" PKR"<<setw(14)<<right<<order_quantities[i];
            cout << endl;
            for(int i=0;i<60;i++)
            {
                cout<<"-";               
...
void deleteorders(string order_names[],float order_prices[],int order_quantities[],int &no_of_orders,string pdtnames[],float pdtprices[],int pdtquantity[],int &number_of_pdts)
{
    if(no_of_orders==0)
    {
    ...
    for(int i=orderdel;i<no_of_orders;i++)
    {
       ...
       order_quantities[i]=order_quantities[i+1];
                    ...
                    if(buyerrequest==1||buyerrequest==2||buyerrequest==3||buyerrequest==4||buyerrequest==5||buyerrequest==6)   //validation
                    {
                        switch(buyerrequest)         //for selecting one from multiple options
                        {
                           case 1:
                            ...
                            case 5:
                            {
                                deleteorders(order_names,order_prices,order_quantities,no_of_orders,pdtnames,pdtprices,pdtquantity,number_of_pdts);
                                break;
                            }
                            case 6:
                            {
